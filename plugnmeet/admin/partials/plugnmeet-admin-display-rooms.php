...
<div class="wrap">
    ...
    <form name="search-form" id="search-form">
        <p class="search-box" style="margin-bottom: 20px">
            ...
            <input type="hidden" id="page_num" value="<?php echo isset($_GET['paged']) ? $_GET['paged'] : 0 ?>">
    ...
    <table class="wp-list-table widefat fixed striped table-view-list posts">
        ...
        <?php foreach ($rooms as $room): ?>
            <tr id="post-<?php echo $room->id ?>"
                class="iedit type-post format-standard">

                <td class="title column-title"><a
                            href="admin.php?page=plugnmeet&task=edit&id=<?php echo $room->id ?>"><?php echo $room->room_title ?></a>
                </td>
                <td class="moderator_pass column-moderator_pass"><?php echo $room->moderator_pass ?></td>
                <td class="attendee_pass column-attendee_pass"><?php echo $room->attendee_pass ?></td>
                <td class="published column-published"><?php echo $room->published ? __("Published", "plugnmeet") : __("Id", "Unpublished") ?>
                    <div class="alignright">
                        <button class="button button-secondary deleteRoom"
                                id="<?php echo $room->id ?>"><?php echo __("Delete", "plugnmeet") ?></button>
                    </div>
                </td>
                <td class="id column-id" style="width: 2.2em;"><?php echo $room->id ?></td>
